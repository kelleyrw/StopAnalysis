...
void SaveResults(TString sfile, double mH, double _limit, double _limitErr, double significance, double pvalue, double rm2s, double rm1s, double rmedian, double rmedianErr, double rmean, double rp1s, double rp2s){
	...
	tree->Branch("rmedianErr", &rmedianErr, "rmedianErr/D");
...
bool GetCLs(double qdata, TTree* tsb, TTree*tb,  double &cls, double &err, double d_quantile){
	...
	if(d_quantile>0 && d_quantile<1){
		...
		qdata = vqb[(pos<(int)vqb.size()?pos:(vqb.size()-1))];
	}
